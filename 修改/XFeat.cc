...
namespace ORB_SLAM3
{
    ...
    MBABlockImpl::MBABlockImpl(int reduction=16) {
    // 初始化卷积层
    ...
    // 初始化激活函数
    sigmoid = register_module("sigmoid", torch::nn::Hardsigmoid());
    // 参数初始化
    _initialize_weights();
    }
...
torch::Tensor MBABlockImpl::forward(torch::Tensor x) {
    ...
    XFeatModel::XFeatModel()
    {
        ...
        block7=torch::nn::Sequential(
            ...
            torch::nn::ReLU(true),
            torch::nn::Conv2dOptions(128, 128, 3).stride(1).padding(1).groups(128),
            torch::nn::BatchNorm2d(128).affine(false),
            torch::nn::ReLU(true)
        );
        ...
        block9=torch::nn::Sequential( torch::nn::Conv2dOptions(128, 128, 1).stride(1).padding(0))
        block10=torch::nn::Sequential(
            torch::nn::Conv2dOptions(256, 128, 3).stride(1).padding(1),
            torch::nn::BatchNorm2d(128).affine(false),
            torch::nn::ReLU(true),
            torch::nn::Conv2dOptions(128, 128, 3).stride(1).padding(1).groups(128),
            torch::nn::BatchNorm2d(128).affine(false),
            torch::nn::ReLU(true),
            torch::nn::Conv2dOptions(128, 64, 1).stride(1).padding(0)
        )

        block11=torch::nn::Sequential(
            torch::nn::Conv2dOptions(128, 64, 3).stride(1).padding(1),
            torch::nn::BatchNorm2d(64).affine(false),
            torch::nn::ReLU(true),
            torch::nn::Conv2dOptions(64, 64, 3).stride(1).padding(1).groups(64),
            torch::nn::BatchNorm2d(64).affine(false),
            torch::nn::ReLU(true)
        )
    ...
    {   
        ...
        */
        ...
        torch::Tensor x6 = torch::nn::functional::interpolate(block9->forward(x5), 
                                                            torch::nn::functional::InterpolateFuncOptions().scale_factor(2.0).mode(torch::kBilinear).align_corners(false));
        x6 = torch::cat({x6, x4}, 1);  // dim=1 表示在通道维度拼接
        torch::Tensor x8 = torch::nn::functional::interpolate(block10->forward(x6), 
                                                            torch::nn::functional::InterpolateFuncOptions().scale_factor(2.0).mode(torch::kBilinear).align_corners(false));
        x8 = torch::cat({x8, x3}, 1);  // dim=1 表示在通道维度拼接
        torch::Tensor x10 = block11->forward(x8);  // 通过 block11
