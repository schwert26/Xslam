...
{   std::cout << "*READY TO LOAD SETTINGS*" << std::endl;
    // Load camera parameters from settings file
    if(settings){
        newParameterLoader(settings);
         std::cout << "SUCCESS TO LOAD SETTINGS*" << std::endl;
    }
    else{
...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
        if (((int)mCurrentFrame.mvKeys.size()<=100)||((mSensor == System::IMU_MONOCULAR)&&(mLastFrame.mTimeStamp-mInitialFrame.mTimeStamp>1.0)))
        {
         ...
         std::cout<<"开始单目初始匹配"<<std::endl;
        // Find correspondences
        ORBmatcher matcher(0.9,true);
        int nmatches = matcher.SearchForInitialization(mInitialFrame,mCurrentFrame,mvbPrevMatched,mvIniMatches,100);
        std::cout<<"匹配个数为"<<nmatches<<std::endl;
        ...
        std::cout<<"立体匹配个数为"<<mpCamera->ReconstructWithTwoViews(mInitialFrame.mvKeysUn,mCurrentFrame.mvKeysUn,mvIniMatches,Tcw,mvIniP3D,vbTriangulated)<<std::endl;
